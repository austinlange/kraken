...
Kraken.prototype.openDatabase = function() {
	...
	if (process.env.REDISTOGO_URL) {
	...
	} else if (process.env.HEROKU_POSTGRESQL_MAROON_URL) {
		console.log("Connecting to postgre: " + process.env.HEROKU_POSTGRESQL_MAROON_URL);
		var pg = require('pg');

		pg.connect(process.env.HEROKU_POSTGRESQL_MAROON_URL, (function(error, client) {
			if (error) {
				console.log(error);
				return;
			}
		...
		console.log("Could not connect to database; no connection URL defined.");
	}
};
